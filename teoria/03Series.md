...
<div class="def"> **Definición. La serie armónica generalizada.**
...
Sea ahora  $\alpha >1$. Trataremos de acotar los términos de la sucesión que genera la serie armónica generalizada por los términos de una sucesión que genere una serie convergente, en particular por una serie geométrica de razón $\dfrac{1}{2^{\alpha-1}}$. Para cualquier $k >1$ existe un $p \in \mathbb{N}$ tal que $2^p \leq k < 2^{p+1}$, por lo que será $\dfrac{1}{2^{p+1}} < \dfrac{1}{k} \leq \dfrac{1}{2^p}$, por lo tanto si sustituimos los términos de la sucesión armónica generalizada $\dfrac{1}{k}$ por el correspondiente $\dfrac{1}{2^p}$, tal que $\dfrac{1}{k} \leq \dfrac{1}{2^p}$, entonces tendremos la serie
$$
...
2. Si el $\lim n \cdot a_n$ existe y es positivo, podemos determinar una constante positiva $M$ tal que $na_n>M$ para todo $n>n_0$ y, por lo tanto, $a_n$ está minorada por la sucesión $\{\dfrac{1}{n}\}$ que genera una serie divergente.
...
## La serie de recíprocos de la sucesión de Fibonacci.
<div class="example">
En el siguiente link, `Wolfram Alpha` nos calcula aproximadamente la suma anterior $\sum_{k=0}^\infty \frac{1}{a_k}$:
<l class="center">
...
Finalmente $\lim S_n= L$, puesto que $S_{2n} \leq S_n \leq S_{2n+1}$ y, por lo tanto, $\sum (-1)^{n+1} a_n$ es convergente.
...
$$
\lim \dfrac{|P(n+1)a_{n+1}|}{|P(n)a_n|}=|r|.
$$
</div>
...
Ejemplo 1. Sumar la serie $\sum_{n\geq 1} (-1)^{n+1}\dfrac{n}{5^n}$.
...
Ejemplo 2. Sumar la serie $\sum_{n\geq 1} (-1)^{n+1}\dfrac{n^2}{5^n}$.
...
Si $\sum_{n \geq 1} a_n = \sum_{n\geq 1} (b_n - b_{n+1})$, entonces la serie es convergente si, y sólo sí, la sucesión $\{b_n\}$ es convergente. En cuyo caso $\sum_{n\geq 1} a_n = b_1 - \lim b_n$.
...
**Ejemplo 1**. La serie $\sum_{n \geq 1} \dfrac{1}{(n+1)(n+2)}$ es telescópica, puesto que, como es fácil comprobar
$$
...
<div class="example">
**Ejemplo 2**. Sumar la serie $\sum_{n \geq 1} \dfrac{a-1}{a^{n+1}}$, con $a>1$.

Se trata de una serie telescópica, puesto $\sum_{n \geq 1} \dfrac{a-1}{a^{n+1}}= \sum_{n \geq 1} \left(\dfrac{1}{a^n} - \dfrac{1}{a^{n+1}}\right)$. Dado que $\lim \dfrac{1}{a^{n+1}}=0$, la serie es convergente y su suma es 
$$
