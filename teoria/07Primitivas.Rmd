...
* $\displaystyle \int \sin (m x)\cos (n x)\, dx$, donde $m$ y $n$ son números naturales.
* $\displaystyle \int R(\sin x, \cos x)\, dx$ donde $R(\sin x,\cos x)$ quiere decir una función racional en las variables $\sin x$ y $\cos x$ como por ejemplo $R(\sin x,\cos x)=\frac{\sin^2+\cos x+1}{\cos^2 x-\cos x-1}$.

## Introducción
Recordemos las identidades trigonométricas que usaremos en el cálculo de las integrales anteriores:

* El seno del ángulo doble: $\sin (2x)=2\sin x\cos x$.
* El coseno del ángulo doble: $\cos (2x)=\cos^2 x-\sin^2 x=2\cos^2 x-1=1-2\sin^2 x$. De las identidades anteriores tenemos que $\sin^2 x=\frac{1}{2}(1-\cos (2x)),\ \cos^2 x=\frac{1}{2}(1+\cos (2x))$.
* Relación entre el coseno y la tangente: $1+\tan^2 x=\frac{1}{\cos^2 x}$, o, si se quiere $\tan^2 x=\sec^2 x-1$ o, $\cot^2 x=\csc^2x -1$.
...
* 1: Uno de los números $n$ o $m$ es cero. En este caso, distinguimos los subcasos siguientes:
  * 1.1: $m$ impar. En este caso, existe un natural $m'$ tal que $m=2m'+1$. La integral indefinida a calcular sería:
$$
\int\sin^{2m'+1}x\, dx =\int (\sin^2 x)^{m'}\sin x\, dx=\int (1-\cos^2 x)^{m'}\sin x\, dx.
$$

## Integrales indefinidas de $\sin^m x\cos^n x$
A continuación, si desarrollamos la expresión $(1-\cos^2 x)^{m'}$ nos quedará una suma de integrales de la forma $\displaystyle\int \cos^j x\sin x\, dx$, con $j$ natural. Dichas integrales son inmediatas y corresponden a la primera fila de la tabla de integrales inmediatas con $g(x)=\cos x$:
$$
\int \cos^j x\sin x\, dx =-\int \cos^j x (-\sin x)\, dx = -\frac{\cos^{j+1}x}{(j+1)}+C.
$$
  
  * 1.2: $n$ impar. En este caso, existe un natural $n'$ tal que $n=2n'+1$. Este caso es parecido al anterior:
$$
\int\cos^{2n'+1}x\, dx =\int (\cos^2 x)^{n'}\cos x\, dx=\int (1-\sin^2 x)^{n'}\cos x\, dx.
$$


## Integrales indefinidas de $\sin^m x\cos^n x$
A continuación, si desarrollamos la expresión $(1-\sin^2 x)^{n'}$ nos quedará una suma de integrales de la forma $\displaystyle\int \sin^j x\cos x\, dx$, con $j$ natural. Dichas integrales son inmediatas y corresponden a la primera fila de la tabla de integrales inmediatas con $g(x)=\sin x$:
$$
...
## Integrales indefinidas de $\sin^m x\cos^n x$
  * 1.3 $n$ par. En este caso, existe un natural $n'$ tal que $n=2n'$. El cálculo de la integral es el siguiente:
$$
\begin{array}{rl}
\displaystyle\int \cos^{2n'} x\, dx & =\displaystyle \int (cos^2 x)^{n'}\, dx  = \int\left(\frac{1+\cos(2x)}{2}\right)^{n'}\, dx
\\ & = \displaystyle \frac{1}{2^{m'+1}}\int (1+\cos t)^{m'}\, dt,
\end{array}
$$
donde en la última igualdad hemos hecho el cambio de variable $t=2x$. 

## Integrales indefinidas de $\sin^m x\cos^n x$
Seguidamente si desarrollamos $(1+\cos t)^{m'}$ nos van saliento integrales del tipo $\displaystyle \int \cos^j x\, dx$ donde si $j$ es impar, ya las hemos estudiado en el apartado 1.2 y si $j$ es par, hemos reducido el exponente a la mitad. En el peor de los casos nos saldría la integral $\displaystyle\int\cos^2 x\,dx$ que se resolvería de la forma siguiente:
$$
\begin{array}{rl}
...
## Integrales indefinidas de $\sin^m x\cos^n x$
  ...
  * 1.4 $m$ par. En este caso, existe un natural $m'$ tal que $m=2m'$. El cálculo de la integral es el siguiente:
$$
\begin{array}{rl}
\displaystyle\int \sin^{2n'} x\, dx & =\displaystyle \int (sin^2 x)^{n'}\, dx  = \int\left(\frac{1-\cos(2x)}{2}\right)^{n'}\, dx
\\ & = \displaystyle \frac{1}{2^{m'+1}}\int (1-\cos t)^{m'}\, dt,
\end{array}
$$
donde en la última igualdad hemos hecho el cambio de variable $t=2x$.  Seguidamente si desarrollamos $(1+\cos t)^{m'}$ nos van saliento integrales del tipo $\displaystyle \int \cos^j x\, dx$ que ya están estudiadas en los apartados 1.2 y 1.3.

## Integrales indefinidas de $\sin^m x\cos^n x$
* 2: Uno de los números $n$ o $m$ es par y el otro, $impar$. 
  * 2.1: $m$ es impar y $n$ es par. En este caso, existen dos naturales más, $m'$ y $n'$ tal que $m=2m'+1$ y $n=2n'$. Para resolver la integral inmediata, hacemos lo siguiente:
$$
\begin{array}{rl}
\displaystyle\int \sin^{2m'+1}x\cos^{2n'}x\, dx & \displaystyle =\int (\sin^2 x)^{m'}\cos^{2n'} x \sin x\, dx \\ & \displaystyle =\int (1-\cos^2 x)^{m'}\cos^{2n'}x \sin x\, dx.
\end{array}
$$
Seguidamente si desarrollamos $(1-\cos^2 x)^{m'}$ nos van saliento integrales del tipo $\displaystyle \int \cos^j x\sin x\, dx$, que son inmediatas, ver apartado 1.1.

## Integrales indefinidas de $\sin^m x\cos^n x$

  * 2.2: $m$ par y $n$ impar. En este caso, existen dos naturales más, $m'$ y $n'$ tal que $m=2m'$ y $n=2n'+1$. Para resolver la integral inmediata, hacemos lo siguiente:
$$
\begin{array}{rl}
\displaystyle\int \sin^{2m'}x\cos^{2n'+1}x\, dx & \displaystyle =\int \sin^{2m'}x (\cos^2 x)^{n'} x \cos x\, dx \\ & \displaystyle =\int \sin^{2m'}x (1-\sin^2 x)^{n'} \cos x\, dx.
\end{array}
$$
Seguidamente si desarrollamos $(1-\sin^2 x)^{m'}$ nos van saliento integrales del tipo $\displaystyle \int \sin^j x\cos x\, dx$ que son inmediatas, ver apartado 1.2.


## Integrales indefinidas de $\sin^m x\cos^n x$
* 3: Las dos potencias $m$ y $n$ son pares. En este caso, existen dos naturales más, $m'$ y $n'$ tal que $m=2m'$ y $n=2n'$. Para resolver la integral inmediata, hacemos lo siguiente:
$$
\begin{array}{rl}
\displaystyle\int \sin^{2m'}x\cos^{2n'}x\, dx & \displaystyle =\int (\sin^2 x)^{m'}\cos^{2n'} x \, dx \\ & \displaystyle =\int \left(\frac{1-\cos(2x)}{2}\right)^{m'}\cdot\left(\frac{1+\cos (2x)}{2}\right)^{n'}\, dx \\ & \displaystyle =\frac{1}{2^{m'+n'+1}}\int (1-\cos t)^{m'}\cdot (1+\cos t)^{n'}\, dt
\end{array}
$$
...
## Integrales indefinidas de $\sin^m x\cos^n x$
Si desarrollamos las expresiones $(1-\cos t)^{m'}\cdot (1+\cos t)^{n'}$ nos quedarán sumas de integrales del tipo $\displaystyle \int \cos^j t\, dx$ que ya han sido estudiadas en los apartados 1.2 y 1.3. 

* 4: Las dos potencias $m$ y $n$ son impares. En este caso, existen dos naturales más, $m'$ y $n'$ tal que $m=2m'+1$ y $n=2n'+1$. Para resolver la integral inmediata, hacemos lo siguiente:
$$
\begin{array}{rl}
\displaystyle\int \sin^{2m'+1}x\cos^{2n'+1}x\, dx & \displaystyle =\int \sin^{2m'+1}x (\cos^2 x)^{n'} \cos x\, dx \\ & \displaystyle =\int \sin^{2m'+1}x (1-\sin^2 x)^{n'} \cos x\, dx.
\end{array}
$$

## Integrales indefinidas de $\sin^m x\cos^n x$
A continuación, si desarrollamos la expresión $(1-\sin^2 x)^{n'}$ nos quedará una suma de integrales de la forma $\displaystyle\int \sin^j x\cos x\, dx$, con $j$ natural. Dichas integrales son inmediatas, ver apartado 1.2.
...
$$
\begin{array}{rl}
\displaystyle\int \cot^2 x\, dx  & \displaystyle =-\int -(1+\cot^2 x-1)\, dx = -\int -(1+\cot^2 x)\, dx - \int 1\, dx \\ & \displaystyle =-\cot x-x+C.
\end{array}
$$

## Integrales indefinidas de $\tan^n x$ y $\cot^n x$
Sea un $n\geq 3$ natural. La integral indefinida $\displaystyle\int tan^n x\, dx$ se calcularía de la forma siguiente:
$$
\begin{array}{rl}
