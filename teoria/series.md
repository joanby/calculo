...
$$
\lim \sqrt[n]{\left(\dfrac{an+1}{an}\right)^{n^2}}  = \lim \left(\dfrac{an+1}{an}\right)^n = e^{\lim n\left( 1-  \dfrac{an+1}{an}\right)} = e^{\frac{1}{a}} 
$$
y $e^{\frac{1}{a}}<1$ ssi $a<0$
...
Ejemplo. Hemos visto que el criterio de d'Alembert no servia para determinar la convergencia de la serie $a_n= \dfrac{n!}{(a+1)(a+2) \cdots (a+n)}$, puesto que el $\lim \dfrac{a_{n+1}}{a_n} = \lim \dfrac{n+1}{a+n+1}=1$. Veamos si el criterio de Raabe aporta més información:
$$
...
<div class="example"> **Ejemplo**
 ...
 Dado que la sucesión es de términos positivos y decreciente, podemos aplicar el criterio de condensación. Puesto $2^k a_{2k}= 2^k \dfrac{1}{2^k\sqrt[q]{2^k}}=\left(\dfrac{1}{\sqrt[q]{2}}\right)^k$, la serie "conensada" será
 $$
 \sum \left(\dfrac{1}{\sqrt[q]{2}}\right)^k
 $$
 es decir, una setiue geométrica de razón $\dfrac{1}{\sqrt[q]{2}} < 1$ y, por lo tanto, convergente.
...
Ejemplo 2. La serie $\sum (-1)^n e^{-n}$ es convergente, dado que satisface la condiciones del criterio de Leibniz, es decir, $\lim e^{-n} =0$ y $e^{-n} \geq e^{-(n+1)}$. Se trata de una serie absolutamente convergente, puesto que $\lim \sqrt[n]{e^{-n}}= e^{-1} <1$. Obsérvese que esta serie es una serie geomètrica de razón $-e^{-1}$, y por lo tanto su suma es 
$$
\sum (-1)^n e^{-n} = \dfrac{-1}{1+e}
$$
...
$$
(1+\dfrac{1}{5})S= \sum (-1)^{n+1}\dfrac{2n-1}{5^n}= 2 \cdot \sum (-1)^{n+1}\dfrac{n}{5^n} - \sum (-1)^{n+1}\dfrac{1}{5^n} = 2 \cdot \dfrac{5}{36} - \dfrac{1}{6} = \dfrac{1}{9}
$$
En definitiva
$$
...
$$
Ejemplo 2. Sumar la serie $\sum \dfrac{a-1}{a^{n+1}}$, con $a>1$.

Se trata de una serie telescópica, puesto $\sum \dfrac{a-1}{a^{n+1}}= \sum \left(\dfrac{1}{a^n} - \dfrac{1}{a^{n+1}}\right)$. Dado que $\lim \dfrac{1}{a^{n+1}}=0$, la serie es convergente y su suma es 
$$
