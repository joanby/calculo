...
$0$ es un punto de acumulación del dominio de $F$, pero no existe el $\lim_{x \rightarrow 0} f(x)$, puesto que en todo entorno de $0$ hay puntos $x_1$ tales que $f(x_1)=1$ así como otros, $x_2$ tales que $f(x_2)=-1$. 


## Límites laterales

Entonces, si existiese un $L$ tal que para todo $\epsilon >$ existe $\delta>0$ tal que $|f(x)-L|<\epsilon$ para todo $|x| <\delta$, resultaria que para dos puntos cualesquiera $x_1$ y $x_2$ tales que $|x_1|<\delta$ i $|x_2|<\delta$, seria $|f(x_1)-f(x_2)|= |f(x_1) -L+L-f(x_2)|\leq |f(x_1) -L|+|L-f(x_2)|<2\epsilon$, pero es evidente que si $x_1 >0$ y $x_2 < 0$, entonces $|f(x_1)-f(x_2)| = 2$ y, por lo tanto, esta diferencia no se puede hacer menor que cualquier número positivo.

Sin embargo, como para todo $0<x<\delta$, es $f(x)=1$, resulta que en este caso si que tendríamos $1$ como el límite de la función $f$ cuando sólo consideremos los reales positivos como su dominio.

Análogamente, si sólo consideramos los $-\delta < x <0$, entonces es $f(x)=-1$ y podríamos tomar $-1$ como el límite de $f$ cuando restringimos su dominio a los reales positivos.
...
Sea $f: A \subset \mathbb{R} \rightarrow \mathbb{R}$ una función real de variable real y sea $c$ un punto de acumulación de $A$. $L \in \mathbb{R}$ es el **límite lateral por la derecha** de $f$ en $c$, si para todo $\epsilon >0$, existe $\delta >0$ tal que, siempre que $0<x-c<\delta$, entonces es $|f(x)-L| < \epsilon$. Escribiremos $L = \lim_{x \rightarrow c^+}f(x)$

Análogamente, $L \in \mathbb{R}$ es el **límite lateral por la izquierda** de $f$ en $c$, si para todo $\epsilon >0$, existe $\delta >0$ tal que, siempre que $0<c-x<\delta$, entonces es $|f(x)-L| < \epsilon$. Escribiremos $L = \lim_{x \rightarrow c^-}f(x)$ 
