...
Dado que $f$ es contínua en $[a,b]$, $f$ está acotada, por lo tanto existen $c= \sup_{[a,b]}f$ y $d= \inf_{[a,b]}f$. Veamos que existen $x_1$ y $x_2$ tales que $c=f(x_1)$ y $d=f(x_2)$. Supongamos, en primer lugar que no existe $x$ tal que $f(x) = c$, entonces la función $h(x)=\dfrac{1}{f(x)-c}$ es contínua y por lo tanto, debería estar acotada en ¢[a,b]$.
Veamos que no es así: Supongamos 
