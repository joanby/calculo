...
Cómo hemos demostrado, dado un número irracional y una precisión, siempre es posible determinar un racional que diste del irracional dado menos que dicha precisión. Es lo que hacemos cuando tomamos $\pi$ como $3.14$ o $3.1415$, $\ldots$
...
**Demostración**
  ...
  a) Dado un $\epsilon >0$, existen $n_1,n_2 \in \mathbb{N}$, tales que $|a_1 -a|< \dfrac{\epsilon}{2}$ si $n>n_1$ y $|b_n -b|< \dfrac{\epsilon}{2}$ si $n>n_2$. Entonces, si $n \geq \max\{n_1,n_2\}$, tendremos que: 
...
1) La sucesión $\left\{\dfrac{5-n^2}{n} \right\}$, es una sucesión divergente, puesto que los números naturales no están acotados en $\mathbb{R}$ y, por lo tanto $\lim n = +\infty$.

2) La sucesión $\left\{\dfrac{5-n^2}{n}\right\}$, también es una sucesión divergente, puesto que 
$$
...
<l class="prop">Límite del cociente de dos polinomios en $n$: </l>
$$
...
$$
\lim \dfrac{P(n)}{Q(n)} = \left{
\begin{cases}
0, si h<k; \\
\dfrac{a_h}{b_k}, si h=k; \\
\infty, si h>k.
\end{cases}
\right.
$$
...
## Ejemplos: **El número $e$**
 ...
 $$
puesto que todos los términos -a a partir del primero- s´on mayores que $2$ y menores que $3$. Este límite es lo que conocemos como el número $e$
$$
