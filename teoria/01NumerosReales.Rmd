...
3) **Entorno reducido** de un punto: un entorno abierto (cerrado) sin el punto, es decir $V^*_{\epsilon}(a)=V_{\epsilon}(a) \setminus \{a\}$ $\left(\overline{V}^*_{\epsilon}(a) = \overline{V}_{\epsilon}(a)  \setminus \{a\}\right)$
...
<div class="example"> **Ejemplos**.
  ...
  1) $1$ es una cota inferior del conjunto de los números naturales.
  
  2) El conjunto $A =\{q \in \mathbb{Q}$ tal que $q^2 \leq 2 \}$ está acotado superiormente. 
  
Tambien lo está inferiormente. 
  
Obsérvese que la existencia de una cota superior  supone la existencia de infinitas cotas, puesto que cualquier número mayor también será una cota superior. Análogamente para las cotas inferiores.
...
Consideremos ahora $\epsilon = \min \left\{1,\dfrac{2-p^2}{2p+1} \right\}$, entonces 
...
Sean $n \in \mathbb{N}$ y $N_n$ el conjunto de los $n$ primeros números naturales. La aplicación $f(k)=k$ de $N_n \rightarrow \mathbb{N}$ es inyectiva, pero no exhaustiva, ya que cualquier natural  mayor que $n$ no es  imagen de ningún elemento de $N_n$.
</div>
