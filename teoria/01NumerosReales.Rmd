...
1) **Entorno abierto:** Dados  un punto $a \in \mathbb{R}$ y un $\epsilon >0$, el **entorno abierto** de centro $a$ y radio $\epsilon$ es el conjunto de puntos $V_{\epsilon}(a)=\{ x \in \mathbb{R}: |x-a|<\epsilon \} = (a-\epsilon, a+\epsilon)$. 

2) **Entorno cerrado:** Dados  un punto $a \in \mathbb{R}$ y un $\epsilon >0$, el **entorno cerrado** de centro $a$ y radio $\epsilon$ es el conjunto de puntos $\overline{V}_{\epsilon}(a)=\{ x \in \mathbb{R}: |x-a| \leq \epsilon \} = [a-\epsilon, a+\epsilon]$. 

3) **Entorno reducido** de un punto: un entorno abierto (cerrado) sin el punto, es decir $V^*_{\epsilon}(a)=V_{\epsilon}(a) \setminus \{a\}$ ($\overline{V}^*_{\epsilon}(a)  \setminus \{a\}$)
...
Para lo que sigue, es conviente recordar que una **aplicación**, $f$ de un conjunto $A$ en otro $B$, representada por $f: A \rightarrow B$, es una asignación de elementos de $B$ a elementos de $A$. de tal manera a que cada elemento de $A$ se le asigna un, y sólo un, elemento de $B$. 
...
Sean $n \in \mathbb{N}$ y $N_n$ el conjunto de los $n$ primeros números naturales. La aplicación $f(k)=k$ es inyectiva, pero no exhaustiva, ya que cualquier natural  mayor que $n$ no es  imagen de ningún elemento de $N_n$.
</div>
...
Un conjunto **finito** es un conjunto con $n$ elementos. Es decir, un conjunto $A$ es finito si existe una aplicación inyectiva de $A$ en el conjunto de los números naturales $\mathbb{N}$.
...
La técnica aplicada en la demostración anterior, se conoce con el nombre del **Principìo del palomar** y resulta ser extremadamente útil en la resolución de diversos tipos de problemas.

## Conjuntos numerables.

Está claro que dos conjuntos finitos con el mismo número de elementos tienen el mismo cardinal, pero ¿qué pasa con los conjutos infinitos? 
...
El conjunto $\mathbb{Z}$ de los números enteros es numerable. También lo es el conjunto de los números naturales pares o el de los múltiplos de otro número natural $k$.
</div>
...
El conjunto de los números reales, $\mathbb{N}$, no es numerable.

<div class="dem"> **Demostración**

Demostraremos que el intervalo abierto $(0,1) \subset \mathbb{R}$ no es numerable y, por lo tanto, el conjunto $\mathbb{R}$ no podrá ser numerable.

Supongamos que si, que $(0,1)$ es numerable. Es decir que podemos disponer estos números, en su representación decimal, de manera que uno sea el primero, otro el segundo , y así sucesivamente:
$$
...
\ldots
$$
Consideremos ahora el número $a=0.b_1b_2b_3 \ldots b_n \ldots$, en el que cada $b_k=a_{kk}$, si $a_{kk} \neq 8$, y $b_k = 0$, si $a_{kk} = 8$. El número $a$ no está en la lista de números anterior, puesto que si ocupase el lugar $k$ diferiria del correspondiente $a_k$ en la cifra que ocupa el lugar $k$ de este número. 
...
Observación: como se verá más adelante, un conjunto indexado por los números naturales, como lo son $\{I_n\}$, $\{a_n\}$ y $\{b_n\}$, recibe el nombre de **sucesión**

## Intervalos anidados (2)

<l class="proposition"> **Proposición** </l>
...
Hemos visto en la proposición anterior que $\xi = \sup\{a_n: n \in \mathbb{N} \}$ está en todos los $I_n$. Igual tambien que en la proposición anterior se demuestra que $\eta = \inf \{b_n: n \in \mathbb{N} \}$ tambien está en todos los intervalos. Sólo falta comprobar que $\xi = \eta$. Ahora bien:
...
Supongamos que dado un $\epsilon >0$, en el entorno de $\xi$ definido por $(\xi - \epsilon, \xi+\epsilon)$ hay sólo un número finito de puntos de A: $\{a_1, \ldots, a_k\}$ distintos de $\xi$. Sea ahora $\delta = \inf\{ |a_i-\xi|: i=1,\ldots,k\}$. Es evidente que si $x$ es un punto del entorno abierto $(\xi-\delta,\xi+\delta)$, entonces es $|x-\epsilon|<\delta$, por lo que no puede ser uno de los $a_i$ y, por lo tanto, este seria un entorno de $\xi$ sin puntos de $A$ diferentes de $\xi$, lo que contradice que $\xi$ sea un punto de acumulación de $A$.
...
<l class="prop"> **Proposición: Teorema de Bolzano-Weirstrass**
...
Por tener $A$ infinitos puntos, en al menos uno de los intervalos $[-K,0]$ y $[0,K]$ hay infinitos puntos de $A$. Sea $I_1 =[a_1,b_1]$ esta mitad. (Si los hay en los dos, nos quedamos con el de la izquierda, por ejemplo), Observen que $|b_1-a_1|=\dfrac{K}{2}$. 
...
## Teorema de Bolzano-Weirstrass (cont.)
