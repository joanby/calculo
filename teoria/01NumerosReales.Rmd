...
Una sucesión de **intervalos anidados** es una conjunto de intervalos cerrados y acotados de $\mathbb{R}$, $I_n = [a_n,b_n]$, tal que para todo $n \in \mathbb{N}$, es $I_{n+1} \supset I_n$ o, lo que es equivalente, tal que $a_n \leq a_{n+1} \leq b_{n+1} \leq b_n$

Observación: como se verá más adelante, un conjunto indexado por los números natulaes, como lo son $\{I_n\}$, $\{a_n\}$ y $\{b_n\}$, reciben el nombre de **sucesiones**

## Intervalos anidados (2)

<l class="proposition"> **Proposición** </l>

Sea $\{I_n\}_{n \in \mathbb{N}}$ una sucesión de intervalos anidados de $\mathbb{R}$. Entonces existe un $\xi n\in \mathbb{R}$ tal que $\xi \in I_n$ para todo $n \in \mathbb{N}$, o lo que es lo mismo, tal que $a_n \leq \xi \leq b_n$, para todo $n \in  \mathbb{N}$
...
Veremos en el apartado de funciones que, a menudo, tenemos una función definida en todos los puntos próximos a uno dado, pero que no lo está es ese punto. Para resolver este tipo de situaciones será útil lo que sigue.
...
Supongamos que dado un $\epsilon >0$, en el entorno de $\xi$ definido por $(\xi - \epslion, \xi+\epsilon)$ hay sólo un número finito de puntos de A: $\{a_1, \ldots, a_k\}$ distintos de $\xi$. Sea ahora $\delta = \inf\{ |a_i-\xi|: i=1,\ldots,k\}$. Es evidente que si $x$ es un punto del entorno abierto $(\xi-\delta,\xi+\delta)$, entonces es $|x-\epsilon|<\delta$, por lo wue no puede ser una de los $a_i$ y, por lo tanto, este seria un entorno de $\xi$ sin puntos de $A$, lo que contradice que $\xi$ sea un punto de acumulación de $A$.
...
Sea $A\subset \mathbb{R}$ infinito y acotado, veremos que tiene un punto de acumulación. Por estar $A$ acotado, existe $K>0$ tal que $A \subset [-K,K]$. Por tener $A$ infinitos puntos, en al menos uno de los intervalos $[-K,0]$ y $[0,K]$ hay infinitos puntos de $A$. Sea $I_1 =[a_1,b_1]$ esta mitad. (Si los hay en los dos, nos quedamos con el de la izquierda, por ejemplo), observen que $|b_1-a_1|=\dfrac{K}{2}$. Dado que en $I_1 =[a_1,b_1]$ hay infinitos puntos de $A$, podemos repetir el proceso y determinar así un intervalo $I_2 = [a_2,b_2]$, con infinitos puntos de $A$ y tal que $I_2 \subset I_1$ y que $b_2-a_2= \dfrac{b_1 - a_1}{2}= \dfrac{k}{2^2}$. Reiterando este proceso, construiremos una sucesión $I_n$ de intervalos anidados $I_n$, tal que $I_{n+1} \subset I_n$ y $b_n-a_n = \dfrac{K}{2^n}$. Dado que $\inf \{\dfrac{k}{2^n}: n \in \mathbb{N} \} =0$, por el principio de los intervalos anidados, tendremos que hay un solo punto $\xi$ que está en todos los intervalos. $\xi$ es el punto de acumulación buscado, puesto que  cualquier entorno de $\xi$ contiene todos los los intervalos $I_n$ a partir de un cierto $n_0$ y, por lo tanto, infinitos puntos de $A$ 
