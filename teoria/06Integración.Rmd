...
Como $U(f,P_n)=L(f,P_n)$, tenemos que $\displaystyle I=\int_a^b f =\lim_{n\to\infty} U(f,P_n)=\lim_{n\to\infty} L(f,P_n)=C\cdot (b-a).$
...
Sea $f:[a,b]\longrightarrow\mathbb{R}$ una función **continua** en el intervalo $[a,b]$ tal que $f(x)\geq 0$ para todo valor $x\in [a,b]$. Supongamos que existe un valor $c\in [a,b]$ tal que $f(c)>0$. Entonces $\displaystyle\int_a^b f >0$.
...
El **Teorema de la media** nos asegura precisamente esto: para cualquier función, siempre es posible encontrar un rectángulo cuya altura estará siempre entre el ínfimo y el supremo de la función cuyas áreas serán la misma.
...
lines(x,f(x),col="red")
text(pi,1.3,"f(x)")
lines(c(-2,-2),c(0,8/3),col="blue")
lines(c(-2,2),c(8/3,8/3),col="blue")
lines(c(2,2),c(0,8/3),col="blue")
text(-2,-0.2,"a")
text(2,-0.2,"b")
text(1.5,2.1,"f(x)")
...
## Teorema de la media generalizado
<l class="prop">Teorema de la media generalizado.</l>
Sean $f,g:[a,b]\longrightarrow \mathbb{R}$ dos funciones **integrables** en el intervalo $[a,b]$, donde suponemos que $g(x)\geq 0$ para todo valor $x\in [a,b]$ y sea $\displaystyle m=\inf_{x\in [a,b]} f(x)$ y $\displaystyle M=\sup_{x\in [a,b]} f(x)$. Entonces siempre es posible hallar un valor $\lambda\in [m,M]$ tal que $\displaystyle \int_a^b f\cdot g =\lambda\cdot \int_a^b g$.

## Teorema de la media generalizado
<l class="observ">Observación.</l>
El teorema anterior generaliza el teorema de la media ya que basta considerar $g(x)=1\geq 0$ y obtenemos el Teorema de la media para integrales.
Además, igual que pasaba con el **Teorema de la media**, observar que si la función $f$, además de ser **integrable**, es **continua**, podemos decir por el Teorema del valor intermedio que existe un valor $c\in [a,b]$ tal que $\lambda =f(c)$. Entonces, en este caso, podemos escribir la integral de $f\cdot g$ en $[a,b]$ como: $\displaystyle \int_a^b f\cdot g=f(c)\cdot \int_a^b g$.
...
El **Teorema Fundamental del Cálculo** nos resuelve este problema relacionando el problema de **integrar** con **derivar**. 
...
El problema es que nosotros tenemos la función $f(x)$ y queremos hallar precisamente la función que da el área $F(x)$. A la función $F(x)$ se le llama **primitiva** de la función $f(x)$ o **antiderivada** ya que si $F'(x)=f(x)$, para hallar $F(x)$ a partir de $f(x)$ hemos de realizar la operación contraria de derivar.

Gracias a dicho Teorema, de cara a hallar áreas, hemos de aprender a hallar **primitivas**. En el capítulo siguiente, vamos a aprender a hallar primitivas de las funciones más conocidas.
...
$$
Usando el Teorema de la media, podemos afirmar que existe un valor $\lambda\in [\inf f,\sup f]$ tal que:
...
Sea $x$ un valor cualquiera del intervalo $[a,b]$. Si $y$ es otro valor cualquiera con $y\geq x$, el **Teorema de la media** nos dice que existe un valor $c\in (x,y)$ tal que:
$$
...
## Regla de Barrow
La **Regla de Barrow** nos da la expresión del **área** o la **integral** de la función $f$ en el intervalo $[a,b]$ usando el **Teorema fundamental del cálculo**:
...
<l class="observ">Observación.</l>
Si usamos la expresión anterior para $x=b$ obtenemos la expresión del **área** de la función $f$ entre $a$ y $b$:
...
Usando el **Teorema fundamental del cálculo** vamos a generalizar el **Teorema de la media generalizado** en el siguiente sentido:
...
<l class="prop">Segundo Teorema de la media.</l>
Sea $f:[a,b]\longrightarrow \mathbb{R}$ una función integrable en $[a,b]$ y sea $g:[a,b]\longrightarrow\mathbb{R}$ una función monótona, creciente y no negativa. Entonces, existe un valor $c\in [a,b]$ tal que:
$$
...
Indicación: considerar $\tilde{g}(x)=g(b)-g(x)$ y aplicar el segundo teorema de la media.
</div>
...
* Para realizar la integral $\displaystyle\int_a^b f(x)\, dx$, tenemos que descomponer  "lo que integramos", $f(x)\, dx$ en dos partes de la forma $f(x)\, dx=u(x)\cdot dv(x)$. 
...
* Relación entre los diferenciales: $dx=\frac{1}{2}(\sinh t)'=\frac{1}{2}\cosh t$.
...
lines(x,f(x),col="red")
for (i in 1:length(x)){
  lines(c(x[i],x[i]),c(0,f(x[i])),col="blue")}
...
for (i in 1:length(xmenys)){lines(c(xmenys[i],xmenys[i]),c(0,f(xmenys[i])),col="blue")}
for (i in 1:length(xmes)){lines(c(xmes[i],xmes[i]),c(0,f(xmes[i])),col="red")}
```
</div>
...
#for (i in 1:length(xmenys)){lines(c(xmenys[i],xmenys[i]),c(f(xmenys[i]),g(xmenys[i])),col="green")}
#for (i in 1:length(xmes)){lines(c(xmes[i],xmes[i]),c(g(xmes[i]),f(xmes[i])),col="green")}
```
</div>
...
$$
Fijaos que hemos integrado la función $g(x)-f(x)$ en aquellas zonas donde $g(x)\geq f(x)$ y hemos integrado $f(x)-g(x)$ en aquellas zonas donde $f(x)\geq g(x)$ para que el área siempre se sume y sea positiva.
...
## Introducción
Vayamos por partes, ¿cómo se realizará dicha generalización? Pues usando el concepto de límite:
...
lines(x,f(x),col="red")
lines(c(xmax/16,xmax/16),c(0,f(xmax/16)),col="blue",lty=2)
for (i in 1:length(x)){
  lines(c(x[i],x[i]),c(0,f(x[i])),col="blue")}
...
* si $g$ está acotada, sea $M=\sup\{g(x),\ |\ x\in [a,\infty)\}$. Entonces $\displaystyle \lim_{x\to\infty} g(x)=M$.
...
$$
Fijaos que si demostramos lo anterior, podemos deducir que $i\Rightarrow j$ para todo $i,j\in \{1,2,3,4\}$.
...
La integral en función de la nueva variable $t$ será:
$$
\int_{b_1}^{b_2}\sin(x^2)\,dx =\int_{b_1^2}^{b_2^2}\sin t \frac{1}{2\sqrt{t}}\, dt.
$$
...
$$
\int_{b_1^2}^{b_2^2}\sin t \frac{1}{2\sqrt{t}}\, dt =\left[\frac{1}{2\sqrt{t}}\cdot (-\cos t)\right]_{b_1^2}^{b_2^2}-\frac{1}{4}\int_{b_1^2}^{b_2^2} \cos t\cdot t^{-\frac{3}{2}}\, dt= \frac{\cos(b_1^2)}{2 b_1}-\frac{\cos(b_2^2)}{2 b_2}-\frac{1}{4}\int_{b_1^2}^{b_2^2}\frac{\cos t}{t^{\frac{3}{2}}}\, dt.
$$
...
## Criterios de convergencia
<l class="prop">Teorema: criterio integral de convergencia de una serie de números.</l>
...
Como la serie es de números positivos, para ver que es convergente, basta demostrar que la sucesión de las sumas parciales está acotada.
...
<l class="definition">Convergencia absoluta.</l>
...
## Criterios de convergencia
Los **criterios de convergencia de Leibnitz** y de **Abel-Dirichlet** son claves para estudiar la convergencia de integrales impropias cuya función puede cambiar de signo.


## Criterios de convergencia
<l class="prop">Teorema. Criterio de convergencia de Leibnitz.</l>
...
Por el criterio de Leibnitz de convergencia de series alternadas, la serie alternada $\displaystyle\sum_{n=1}^\infty b_n$ es convergente.
...
$$
Como $\displaystyle\lim_{n\to\infty}\ln\left(\frac{n+1}{n}\right)=0$, por el criterio del sandwitz, $\displaystyle\lim_{n\to\infty}|b_n|=0$.
...
## Criterios de convergencia
<div class="example">
Por el criterio de Leibnitz de series alternadas, la serie $\displaystyle \sum_{n=1}^\infty b_n$ es convergente y por el criterio de convergencia de Leitnitz, la integral impropia $\displaystyle\int_1^\infty \frac{\sin x}{x}\, dx$ también será convergente.
...
Como $\displaystyle \lim_{x\to\infty} g(x)=0$, tendremos que $\displaystyle\lim_{b_1,b_2\to\infty} 2C\max{|g(b_1)|,|g(b_2)|}=0$ y, por el criterio del sandwitz, $\displaystyle\lim_{b_1,b_2\to\infty} |g(b_2)\cdot S(b_2)-g(b_1)\cdot S(b_1)|=0$.
...
## Integración por cambio de variable en una integral impropia
...
## Integración por partes en una integral impropia
