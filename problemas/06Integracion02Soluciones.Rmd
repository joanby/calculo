...
Antes de exponer la solución del problema, veamos el resultado siguiente:

\begin{theorem}
Sea $f:D\longrightarrow\mathbb{R}$ una función continua siendo $D$ su dominio de definición. Sean $g,h:[a,b]\longrightarrow\mathbb{R}$ funciones derivables tales que $g([a,b])\subseteq D$, $h([a,b])\subseteq D$. Definimos la función 
$$
F(x)=\int_{g(x)}^{h(x)} f(t)\,dt.
$$
Entonces $F$ es derivable y el valor de su función derivada $F'(x)$ puede expresarse como:
$$
...
Veamos pues que $\displaystyle H'(x)=f(h(x))\cdot h'(x)$. Notemos en primer lugar que podemos escribir la función $H(x)$ como la composición de las funciones siguientes:
...
Resolvamos el ejercicio usando la proposición anterior:
