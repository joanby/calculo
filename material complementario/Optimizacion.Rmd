...
* Podemos suponer que $0\leq x\leq d$ ya que 
  ...
  * si $x<0$ o el punto $X$ está a la derecha del punto $A$, el tiempo que tardará el excursionista de ir desde $A$ hasta $X$ más el tiempo de ir desde $X$ a $B$ seguro que superará el tiempo de ir desde $A$ hasta $A'$ y desde $A'$ hasta $B$.
...
* El ángulo $\alpha$ con que atraviesa el río tiene como tangente será:
  * si $x=\frac{h}{\sqrt{k^2-1}}<d$, el ángulo $\alpha$ tiene como tangente:
   $$
   \tan\alpha =\frac{h}{x}=\frac{h}{\frac{h}{\sqrt{k^2-1}}}=\sqrt{k^2-1},\ \Rightarrow\alpha =\arctan(k^2-1).
   $$
...
* El ángulo $\alpha$ será en este caso,
$$
\alpha =\arctan(k^2-1)=\arctan\sqrt{3}=\frac{\pi}{3}\ \mbox{radianes}=60\ \mbox{grados.}
$$